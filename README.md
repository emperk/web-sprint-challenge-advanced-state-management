...
1. What problem does the context API help solve?
  ANSWER: Context API is meant to help with sharing state down a component tree. It helps prevent prop drilling.
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  ANSWER: Actions (aka events) are the only way you can send data from your app to your Redux store. Reducers are functions that determines changes to an application's state. It takes the action it receives to determine this change. The store holds all information in the app. It also manages all the data. The store is known as a 'single source of truth' because the only way to change your data in UI is to dispatch redux action which changes state within the redux reducer.
...
4. What is your favorite state management system you've learned and this sprint? Please explain why!
  ANSWER: I actually like Redux because while the syntax is a pain to learn, it does make state & data a LOT easier to manage.
